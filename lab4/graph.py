...
def graph(func, lin_approx, qd_approx, qub_approx, exp_approx, log_approx, pow_approx):
    ...
    x = np.linspace(min_x - 0.6, max_x + 0.6, 10000)
    ...
    ax.plot(x, qd_approx(x), "g", linewidth=2.0, label="quad")
    ax.plot(x, qub_approx(x), "b", linewidth=2.0, label="cube")
    if exp_approx is not None:
        ax.plot(x, exp_approx(x), "black", linewidth=2.0, label="exp")
    ...
    if pow_approx is not None:
        ax.plot(x, pow_approx(x), "purple", linewidth=2.0, label="pow")
    ax.legend()
    ax.plot(x_args, y_args, linewidth=0, marker=".", markersize=10, markeredgecolor="black",
